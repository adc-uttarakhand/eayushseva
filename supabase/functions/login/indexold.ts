    ...
    const { data: staffDataList, error: staffError } = await supabaseClient
      ...
      if (firstStaff.login_password === password || firstStaff.password === password) {
        ...
        if (allLinks.length > 1) {
          ...
          const options = await Promise.all(allLinks.map(async l => {
            const hosp = hospitals?.find(h => h.hospital_id === l.hospitalId);
            const userDetails = { role: 'STAFF', id: l.staffRecord.id.toString(), hospitalId: l.hospitalId };
            const token = await generateJWT(userDetails);
            return {
          ...
          const userDetails = { role: 'STAFF', id: staffRecord.id.toString(), hospitalId: singleLink.hospitalId };
          const token = await generateJWT(userDetails);
          // Return safe record
          return new Response(
