...
const HospitalSearchInput = ({ value, onChange, hospitals, placeholder = 'Search hospital...', className = '' }: any) => {
...
const A4Preview = ({
  ...
  return (
    <div style={{
      ...
      {/* Section 2: Present Posting Details */}
      <Section title="2. Present Posting Details">
        <div style={grid2}>
      ...
      <Section title="3. Application Type &amp; Category">
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '8px' }}>
          {APPLICATION_TYPES.map(type => (
            <span key={type} style={{
              border: type === applicationType ? '1.5px solid #1a7a4a' : '1.5px solid #ccc',
              background: type === applicationType ? '#e6f7ef' : 'white',
              color: type === applicationType ? '#1a7a4a' : '#888',
              borderRadius: '20px',
              padding: '3px 10px',
              fontSize: '9.5px',
              fontWeight: type === applicationType ? 'bold' : 'normal'
            }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '10px' }}>
            <thead>
              <tr>
                ...
                <th style={{ background: '#f0f0f0', border: '1px solid #ddd', padding: '4px 6px', textAlign: 'center', width: '65px', fontSize: '9px', color: '#555' }}>Type</th>
                <th style={{ background: '#f0f0f0', border: '1px solid #ddd', padding: '4px 6px', textAlign: 'center', width: '70px', fontSize: '9px', color: '#555' }}>Above 7000ft</th>
              </tr>
            </thead>
            <tbody>
              {Array.from({ length: 10 }, (_, i) => {
                ...
                return (
                  <tr key={i}>
                    <td style={{ border: '1px solid #ddd', padding: '4px 6px', textAlign: 'center', color: '#888', fontWeight: 'bold' }}>{i + 1}</td>
                    <td style={{ border: '1px solid #ddd', padding: '4px 6px', color: choiceName ? '#1a1a1a' : '#bbb', fontStyle: choiceName ? 'normal' : 'italic', fontWeight: choiceName ? 500 : 400 }}>
                      {choiceName || '—'}
                    </td>
                    ...
                    <td style={{ border: '1px solid #ddd', padding: '4px 6px', textAlign: 'center', color: hosp ? '#555' : '#bbb' }}>
      ...
      <Section title="5. Declaration">
        <div style={{
          ...
          <strong style={{ color: '#1a7a4a' }}>Main Ghoshana Karta/Karti Hoon Ki</strong> — Mujhe malum hai ki galat jaankari dene par mera Sthananataran Prarthna Patra niraast kiya ja sakta hai aur Camp Sewa Niyamon ke antargat vibhagiya karyawahi bhi ki ja sakti hai.
          <br /><br />
...
export default function Sthananataran({ session, profile }: { session?: any; profile?: any }) {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      const canvas = await html2canvas(a4PreviewRef.current, {
        scale: 2,
        useCORS: true,
      ...
      // Always fit entire content into one A4 page — shrink if needed
      const canvasRatio = canvas.height / canvas.width;
      const imgHeightInPdf = pdfWidth * canvasRatio;
      if (imgHeightInPdf <= pdfHeight) {
        // Content fits naturally — place as-is
        pdf.addImage(imgData, 'PNG', 0, 0, pdfWidth, imgHeightInPdf);
      } else {
        // Content is taller — scale down to fit exactly one page
        const scaledWidth = pdfWidth * (pdfHeight / imgHeightInPdf);
        const xOffset = (pdfWidth - scaledWidth) / 2; // center horizontally
        pdf.addImage(imgData, 'PNG', xOffset, 0, scaledWidth, pdfHeight);
      }

      const fileName = `Transfer_Application_${applicantName.replace(/\s+/g, '_') || 'Form'}_${new Date().getFullYear()}.pdf`;
      pdf.save(fileName);
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div>
                ...
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all" placeholder="Enter your email" required />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="text" value={fatherHusbandName} onChange={(e) => setFatherHusbandName(e.target.value)} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all" placeholder="Father or Husband's Name" />
              </div>
              <div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
              {APPLICATION_TYPES.map((type) => (
                <div key={type}
                  onClick={() => { setApplicationType(type); if (type !== 'Anivarya Sthananataran') setMandatoryTransferSubOption(''); }}
                  className={`cursor-pointer border-2 rounded-xl p-4 transition-all ${applicationType === type ? 'border-emerald-500 bg-emerald-50 text-emerald-800 shadow-sm' : 'border-slate-200 hover:border-emerald-300 hover:bg-slate-50 text-slate-600'}`}
                >
                  <div className="flex items-center gap-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50 p-6 rounded-xl border border-slate-100">
              {applicationType === 'Need Exemption from Transfer' && (
                <div>
                  ...
                  <select value={exemptionCategory} onChange={(e) => setExemptionCategory(e.target.value)} className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all">
              ...
              {applicationType === 'Anurodh' && (
                <div>
                  ...
                  <select value={transferCategory} onChange={(e) => setTransferCategory(e.target.value)} className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all">
                  ...
                  <div>
                    ...
                    <div className="flex gap-4">
                      {['Sugam to Durgam', 'Durgam to Sugam'].map(option => (
                        <button key={option} type="button" onClick={() => setMandatoryTransferSubOption(option)}
                          className={`px-4 py-2 rounded-lg text-sm font-semibold border ${mandatoryTransferSubOption === option ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-slate-700 border-slate-300'}`}>
                          {option}
                        </button>
                      ))}
                    </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
            ...
            <div className="mb-8 p-6 bg-slate-50 rounded-xl border border-slate-200">
              ...
              <input type="file" onChange={(e) => setVikalpPatra(e.target.files?.[0] || null)}
                className="w-full px-4 py-2 bg-white border border-slate-200 rounded-lg text-sm file:mr-4 file:py-2.5 file:px-6 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-emerald-600 file:text-white hover:file:bg-emerald-700 transition-all cursor-pointer" required />
            </div>
            ...
            {(applicationType === 'Need Exemption from Transfer' || applicationType === 'Anurodh') && (
              <div className="mb-8 p-6 bg-slate-50 rounded-xl border border-slate-200">
                ...
                <input type="file" onChange={(e) => setProofDocument(e.target.files?.[0] || null)}
                  className="w-full px-4 py-2 bg-white border border-slate-200 rounded-lg text-sm file:mr-4 file:py-2.5 file:px-6 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100 transition-all cursor-pointer" />
              </div>
            )}

            <div className="flex items-start gap-4 p-4 rounded-xl hover:bg-slate-50 transition-colors">
              <div className="flex items-center h-6">
                <input id="declaration" type="checkbox" checked={declarationAccepted} onChange={(e) => setDeclarationAccepted(e.target.checked)}
