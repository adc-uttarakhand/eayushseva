import { Building2, BarChart3, LayoutDashboard, Users, Wrench, User, Key, ClipboardList, Truck, ShieldCheck, ArrowUpDown, Activity, Radio, Flag, FileText, Shield, Calculator, BookOpen } from 'lucide-react';
import { motion } from 'motion/react';

export type TabId = 'training' | 'dashboard' | 'hospitals' | 'doctors' | 'tools' | 'profile' | 'eparchi' | 'stats' | 'demands' | 'supply_upload' | 'district_supply' | 'disease_management' | 'role_management' | 'staff_distribution' | 'pharmacy_dashboard' | 'requests' | 'transfer_module' | 'registrations' | 'nearby' | 'rate' | 'transfer_requests' | 'loginDirectory' | 'panchakarma' | 'rapid_tests' | 'patients' | 'sthanantaran' | 'password_reset' | 'admin_management' | 'live' | 'events' | 'form1' | 'id_card' | 'incentive';
...
export default function BottomNav({ active, setActive, role, isTransferEnabled, hasPanchakarma, modules, isIncharge, accessPages = [], hasActiveLiveStream }: BottomNavProps) {
  ...
  const adminTabs = [
    ...
    { id: 'live' as TabId, label: 'Live', icon: Radio },
    { id: 'training' as TabId, label: 'Trainings', icon: BookOpen },
    { id: 'events' as TabId, label: 'Events', icon: Flag },
    ...
    ...(role === 'SUPER_ADMIN' || role === 'STATE_ADMIN' || role === 'DISTRICT_ADMIN' ? [{ id: 'incentive' as TabId, label: 'Incentive', icon: Calculator }] : []),
